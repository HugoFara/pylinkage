...
def draw_revolute_joint(d: draw.Drawing, x: float, y: float, radius: float = 12):
    ...
    # Outer circle
    d.append(draw.Circle(x, y, radius, fill="white", stroke="black", stroke_width=2.5))
    # Center dot
    d.append(draw.Circle(x, y, 3, fill="black", stroke="none"))
...
def draw_crank_joint(d: draw.Drawing, x: float, y: float, radius: float = 15, angle: float = 0):
    ...
    # Outer circle
    d.append(draw.Circle(x, y, radius, fill="white", stroke="#2E86AB", stroke_width=3))
    # Center dot
    d.append(draw.Circle(x, y, 4, fill="#2E86AB", stroke="none"))
    # Rotation arrow arc
    ...
    path = draw.Path(stroke="#2E86AB", stroke_width=2, fill="none")
...
def draw_link(
    ...
    if style == "bar":
        ...
        d.append(
            draw.Line(
                x1, y1, x2, y2, stroke="white", stroke_width=1, stroke_dasharray="4,3", opacity=0.7
            )
        )
...
def draw_dimension(
    d: draw.Drawing, x1: float, y1: float, x2: float, y2: float, label: str, offset: float = 30
):
    ...
    mid_x = (dx1 + dx2) / 2
    mid_y = (dy1 + dy2) / 2
    d.append(draw.Text(label, 12, mid_x, mid_y - 5, center=True, fill="#333", font_family="Arial"))
...
def create_fourbar_diagram():
    ...
    # Draw ground line
    d.append(draw.Line(ga[0] - 40, ga[1], gd[0] + 40, gd[1], stroke="#333", stroke_width=3))
    ...
    for (x, y), text, ox, oy in labels:
        d.append(draw.Text(text, 12, x + ox, y + oy, fill="#333", font_family="Arial"))
    ...
    for x, y, text, color in link_labels:
        d.append(draw.Text(text, 11, x, y, fill=color, font_family="Arial", font_weight="bold"))
    ...
    d.append(
        draw.Text(
            "Legend", 12, legend_x, legend_y, fill="#333", font_family="Arial", font_weight="bold"
        )
    )
...
def create_complex_linkage():
    ...
    # Ground line
    d.append(draw.Line(ga[0] - 40, ga[1], gf[0] + 40, gf[1], stroke="#333", stroke_width=3))
    ...
    for start, end, color, style in links:
        draw_link(d, start[0], start[1], end[0], end[1], color=color, width=12, style=style)
    ...
    for (x, y), label, ox, oy in joint_labels:
        d.append(
            draw.Text(
                label, 14, x + ox, y + oy, fill="#333", font_family="Arial", font_weight="bold"
            )
        )
...
def create_with_slider():
    ...
    # Slider rail
    rail_y = jc[1]
    d.append(draw.Line(50, rail_y - 15, width - 50, rail_y - 15, stroke="#333", stroke_width=2))
    d.append(draw.Line(50, rail_y + 15, width - 50, rail_y + 15, stroke="#333", stroke_width=2))
