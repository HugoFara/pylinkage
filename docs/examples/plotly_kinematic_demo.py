...
CRANK_SVG = (
    "M 0 -10 A 10 10 0 1 1 0 10 A 10 10 0 1 1 0 -10 Z M 0 -3 A 3 3 0 1 1 0 3 A 3 3 0 1 1 0 -3 Z"
)
...
def create_kinematic_diagram():
    ...
    for x, y, name, _svg_path, color, size in joints_data:
        fig.add_trace(
            go.Scatter(
                x=[x],
                y=[y],
                mode="markers",
                marker=dict(
                    symbol="circle", size=size, color="white", line=dict(color=color, width=2)
                ),
                name=name,
    ...
    fig.update_layout(
        title=dict(text="Four-Bar Linkage - Kinematic Diagram", font=dict(size=20, family="Arial")),
        showlegend=True,
...
def create_animated_linkage():
    ...
    for i, angle in enumerate(angles):
        ...
        joint_c = ground_d + rocker_length * np.array(
            [np.cos(angle_db + angle_d), np.sin(angle_db + angle_d)]
        )
