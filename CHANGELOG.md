...
- **`kinematic_minimization` / `kinematic_maximization` score the linkage the
  ...
  run — which started from wherever the sweep had left the linkage. A long
  stride can hop over a position the linkage cannot take, or onto the other
  ...
  It keeps `dt=1`, the stride `step()` and the visualizers use, because
  which branch the solver follows past a toggle depends on the stride. An
  evaluation therefore costs `get_rotation_period()` steps (at least 96):
  360 for the default crank, and the least common multiple of the periods
  when several actuators drive the linkage.
